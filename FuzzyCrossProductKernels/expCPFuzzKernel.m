function G = expCPFuzzKernel( X,Z,kernelParam, varargin)
%G=expCPFuzzKernel( X,Z,kernelParam): Exponential kernel between fuzzy data X and Z. 
...
%                          exponential cross product kernel on fuzzy sets
%
%G=expCPFuzzKernel( X,Z,kernelParam,option):  option = 0 vectorized form, fast for
%                          fuzzy sets of example IV. option = 1 non vectorized form, fast for fuzzy sets of example V
%
%G=expCPFuzzKernel( X,Z,kernelParam,option,agregation): agregation='sum' uses sum
...
%             X{2}{3,2}=rand(2,1); % MF
%             kernelParam=0025;
%             G=expCPFuzzKernel( X,X,kernelParam)
%
...
%             Z{2}{3,1}=0.1; % MF
%             kernelParam=0025;
%             G=expCPFuzzKernel( X,Z,kernelParam,1)
%
...
%             kernelParam=0025
%             G=expCPFuzzKernel( X,Z,kernelParam,0,'prod')
%
% Example IV
...
%             kernelParam=0025;
%             tic
%             G=expCPFuzzKernel(X,Z,kernelParam,0);
%             t1=toc
%             tic
%             GG=expCPFuzzKernel( X,Z,kernelParam,1 );
%             t2=toc
%             sum(sum(G-GG))
%             [t1, t2]
%
...
switch option
    case 0 % vectorized form
        ...
        for d=1:D
            ...
            GV=cellfun(@(x,z,mx,mz, ix, iz) sum(exp(kernelParam*sum(x(ix,:).*z(iz,:),2)).*mx(ix,:).*mz(iz,:)),XXR, ZZR,MFXR,MFZR,ixx, jzz);
            ...
            for i=1:N
                ...
                for j=1:M
                    ...
                    for ix=1:sizeXX % number of points within each cell
                        ...
                        for jz=1:sizeZZ                           
                            GV(i,j)=GV(i,j)+exp(kernelParam*(XX{i,d}(ix,:)*ZZ{j,d}(jz,:)'))*MFX{i,d}(ix)*MFZ{j,d}(jz);
                        end
                    end
